...
//creating posts
app.post('/profile/create-post', passport.authenticate('jwt', { session: false }), async ( req , res ) => {
    const { title, content } = req.body;

    if (!title || !content) {
        return res.status(400).json({ message: 'Missing required fields: title and content are required.' });
    }
...
app.get('/posts/user/:userId', passport.authenticate('jwt', { session: false }), async (req, res) => {
...
app.post('/post/update/:postId', passport.authenticate('jwt', { session: false }), async (req, res) => {
    ...
    try {
        const updatedPost = await postDb.updatePost(postId, authorId, title, content);

        if (!updatedPost) {
            return res.status(404).json({ message: 'Post not found or user not authorized to edit this post.' });
        }
...
app.get('/post/:postId/comments', passport.authenticate('jwt', { session: false }), async (req, res) => {
...
app.post('/post/:postId/comments', passport.authenticate('jwt', { session: false }), async (req, res) => {
    const { postId } = req.params;  
